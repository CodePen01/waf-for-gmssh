...
from app.consts.settings import (
    PATH_WAF_LUA_INC_CAPTCHA,
    PATH_WAF_LUA_INC_CAPTCHA_NUM1,
    PATH_WAF_LUA_INC_CAPTCHA_NUM2,
)
...
class VerifyCode:
    ...
    __font_path = "./app/static/font.ttf"
...
def make_verify_code():
    ...
    for num in range(1, 51):
        ...
        filename = str(num) + "_" + "".join(codeImage[1]).lower() + ".png"
        filename = f"{PATH_WAF_LUA_INC_CAPTCHA}/{filename}"
        with open(filename, "wb") as f:
            f.write(out.read())

    if send_num1:
        with open(PATH_WAF_LUA_INC_CAPTCHA_NUM1, "w", encoding="utf-8") as f:
            f.write(json.dumps(send_num1))
    if send_num2:
        with open(PATH_WAF_LUA_INC_CAPTCHA_NUM2, "w", encoding="utf-8") as f:
            f.write(json.dumps(send_num2))
